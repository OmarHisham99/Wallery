...
import com.example.wallery.databinding.ActivityStaticImagePageBinding
...
class StaticImagePage : AppCompatActivity() {
    ...
    private val TAG = StaticImagePage::class.java.simpleName
    ...
    private lateinit var staticPageBinding : ActivityStaticImagePageBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_static_image_page)
        tl = Point()
        br = Point()
        openCamera()
        staticPageBinding = DataBindingUtil.setContentView(this, R.layout.activity_static_image_page)

        staticPageBinding.optionsBtn.setOnClickListener{
            menuBehavior(it)
        }
    ...
    private fun menuBehavior(view: View){
        ...
        popupMenu.inflate(R.menu.static_page_menu)
        popupMenu.show()
    }
    ...
    private fun chooseColor() {
        ...
        val colorPicker = AmbilWarnaDialog(this@StaticImagePage, chosenColor, object: AmbilWarnaDialog.OnAmbilWarnaListener {
    ...
    private fun sharePhoto(image:Bitmap){
       val path = MediaStore.Images.Media.insertImage(contentResolver,image, "Shared Photo",null)
    ...
    private fun openCamera() {
        if (ActivityCompat.checkSelfPermission(baseContext, Manifest.permission.CAMERA)
            ...
            if (photoFile != null) {
                val photoURI = FileProvider.getUriForFile(this,"com.example.wallery.provider", photoFile)
...
//                    bitmap = data?.getExtras()?.get("data") as Bitmap
                    staticPageBinding.inputImage.setImageURI(Uri.parse(imageFilePath))

                    bitmap = staticPageBinding.inputImage.drawable.toBitmap()
...
//
        ...
        staticPageBinding.imageFromData.setOnTouchListener(object : View.OnTouchListener {
...
//                    startActivityForResult(i, PICK_FROM_GALLERY)
    ...
    private fun rpPaintHSV(bitmap: Bitmap, p: Point): Mat {
        ...
        showImage(mRgbMat,staticPageBinding.inputImage)
...
//        showImage(mRgbMat,floodFillImage)
        ...
        showImage(result,staticPageBinding.outputImage)
        return result
    }
    ...
    private fun showResultLayouts() {
        staticPageBinding.imageFromData.visibility = View.GONE

        staticPageBinding.topLayout.visibility = View.VISIBLE

        staticPageBinding.bottomLayout.visibility = View.VISIBLE
    }


    private fun showImage() {
        staticPageBinding.imageFromData.visibility = View.VISIBLE
        staticPageBinding.topLayout.visibility = View.GONE

        staticPageBinding.bottomLayout.visibility = View.GONE

        try {
            staticPageBinding.imageFromData.setImageBitmap(bitmap)
        } catch (e: Exception) {
            Toast.makeText(this@StaticImagePage, "No image selected", Toast.LENGTH_SHORT).show()
        }
    }
...
//        Imgproc.medianBlur(mRgbMat,mRgbMat,15)
        ...
        showImage(resultImage,staticPageBinding.outputImage)
    ...
    private fun getTextureImage(): Mat {
        var textureImage = BitmapFactory.decodeResource(getResources(), R.drawable.bricks_texture)
